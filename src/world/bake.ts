 ...
 *
 * **3 is the one exception, and it is deliberate.** It adds the tile flags and changes nothing
 * about the ground: a version-2 world is dropped and regenerated by `worldFor`, and the same seed
 * under the same generator gives the same biomes and bands, so the journey's fog still lines up.
 * Bumping `SAVE_VERSION` would have discarded every journey to protect against a mismatch that only
 * happens if the generator also moved since the world was stored -- and in that case the player
 * gets the current ground, which is what any unreadable bake has always produced. The alternative
 * was keeping a v2 world as it was, which is keeping the Aravali cut in half.
 */
export const BAKE_VERSION = 3;

/**
 * The tile flags a world carries after generation, in bit order.
 *
 * **Version 3 exists because this list did not.** The format was designed when a tile was a biome
 * and a height, and four flags grew on `Tile` afterwards -- `road`, `ford`, `track`, `plank` --
 * none of which was ever written here. So a world came back from storage without them: the first
 * visit to a seed drew its paths and every later visit drew none, and on the Aravali the rail and
 * the planks went too. Those two are the only walkable ground over open water and open sky, so a
 * reload cut off **59% of the map and six of its twelve places** on every seed measured, and a
 * click on any of them silently did nothing.
 *
 * `test/bake.test.ts` did not see it because it compared the fields it knew about by name. It now
 * compares the whole tile, so a fifth flag that is not added here fails by name.
 *
...
function flagsCode(tile: Tile): string {
  let mask = 0;
  TILE_FLAGS.forEach((flag, bit) => {
    if (tile[flag]) mask |= 1 << bit;
  });
...
export interface BakedWorld {
  ...
  bands: string[];
  /** One string per row, one base-36 character per tile: a bitmask over `TILE_FLAGS`. */
  flags: string[];
  start: Point;
...
export function restoreWorld(baked: BakedWorld, fieldMap: FieldMap): FieldMapWorld | null {
  ...
  if (!Array.isArray(baked.biomes) || !Array.isArray(baked.bands) || !Array.isArray(baked.flags)) {
    return null;
  }
  if (
    baked.biomes.length !== baked.height ||
    baked.bands.length !== baked.height ||
    baked.flags.length !== baked.height
  ) {
    return null;
  }
  ...
  for (let y = 0; y < baked.height; y += 1) {
    ...
    const flagRow = baked.flags[y]!;
    if (biomeRow.length !== baked.width || bandRow.length !== baked.width) return null;
    if (flagRow.length !== baked.width) return null;
    const row: Tile[] = [];
    for (let x = 0; x < baked.width; x += 1) {
      const biome = biomeOf.get(biomeRow[x]!);
      if (!biome) return null;
      const mask = Number.parseInt(flagRow[x]!, 36);
      if (!Number.isInteger(mask) || mask >= 1 << TILE_FLAGS.length) return null;
      const tile: Tile = {
        x,
        y,
        biome,
      ...
      // Absent rather than false where unset, which is how the generator leaves them: a flag is
      // only ever written `true`, so a restored tile and a generated one are the same object shape.
      TILE_FLAGS.forEach((flag, bit) => {
        if (mask & (1 << bit)) tile[flag] = true;
      });
      row.push(tile);
    }
    tiles.push(row);
