...
/**
 * The pools used to be built by *repeating* each species by its weight, so that a plain modulo
 * over the expanded array came out weighted. `weightedPickFor` takes the weight directly, so the
 * expansion is gone and with it the last reason a per-biome list's length or order mattered.
 */
function rarityWeight(species: { rarity: Rarity }): number {
  return RARITY_WEIGHT[species.rarity];
}
