...
describe('here', () => {
  ...
  it('offers a night only when one can be spent, and names what kind', () => {
    ...
    const { unmount } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes }} place={{ ...place }}
  ...
  it('shows a tiredness line only when there is one', () => {
    ...
    const { unmount } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes, fatigue: 'You have been walking a while.' }}
            place={{ ...place }} actions={[]} />
    );
    ...
    const { baseElement } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes, fatigue: null }} place={{ ...place }} actions={[]} />
    );
  ...
  it('shows where to go next, and nothing when there is nowhere', () => {
    ...
    const { unmount } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes, whereNext: 'The Camp would do for the night.' }}
            place={{ ...place }} actions={[]} />
    );
    ...
    const { baseElement } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes, whereNext: '' }} place={{ ...place }} actions={[]} />
    );
    expect(baseElement.querySelector('.status-next')).toBeNull();
  });

  it('shows the field notes with no place to stand in', () => {
    render(<Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes }} place={{ ...place }} actions={[]} />);
  ...
  it('gives the slot to the place while it is being read', () => {
    render(<Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes }} place={{ ...place, poiId: 'poi_caravan_camp' }} actions={[]} />);
  ...
  it('says which of the three has the slot, so the stylesheet can ask', () => {
    ...
    const { unmount } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes }} place={{ ...place }} actions={[]} />
    );
    expect(dock()).toBe('notes');
    unmount();

    const second = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes }} place={{ ...place, poiId: 'poi_caravan_camp' }} actions={[]} />
    );
  ...
  it('keeps the actions in reach while a place is being read', () => {
    ...
    render(
      <Here standing={bare} conversation={null}
        height="read"
        onHeight={() => {}}
        open
        notes={{ ...notes }}
  ...
  it('renders nothing at all when the surface is closed', () => {
    const { baseElement } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open={false} notes={{ ...notes }} place={{ ...place, poiId: 'poi_caravan_camp' }} actions={[]} />
    );
  ...
  it('carries canon inside the notes when a service is listening', () => {
    render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={{ ...notes }} place={{ ...place }} canon={<p>Canon says something.</p>} actions={[]} />
    );
...
describe('the people in a place', () => {
  ...
  /**
   * Somebody with no painting, so the silhouette branch is what renders.
   *
   * **Synthetic, and it has to be.** These tests named Thrali until he was painted, at which point
   * they broke; picking whoever was still unpainted fixed that and then quietly ran out — seven of
   * eight are painted now, so the language check had one subject left and the kia and neutral inks
   * were going untested while the suite stayed green. Art arriving should not erode coverage.
   *
   * The trade and language are canon's; only the id is invented, and inventing it is the point —
   * nothing will ever paint `npc_nobody_*`, so the fallback stays exercised forever.
   */
  const nobody = (language: string, role: string) => ({
    id: `npc_nobody_${language || 'none'}`,
    name: 'Somebody',
    role,
    language
  });
  ...
  it('draws a portrait beside every person, coloured by their language', () => {
    ...
    for (const p of people) expect(portraitFor(p.id)).toBeNull();
    ...
    expect(inkOf(0)).toBe(LANGUAGE_INK.kia);
    expect(inkOf(1)).toBe(LANGUAGE_INK.maru);
    expect(Object.values(LANGUAGE_INK)).not.toContain(inkOf(2));
  });
  ...
  it('gives a fisher and a herder different things to hold', () => {
    ...
    render(
      <div>
        <PersonPortrait person={nobody('kia', 'fisher')} />
        <PersonPortrait person={nobody('maru', 'herder')} />
      </div>
    );
...
describe('the field notes draw a mark for every species', () => {
  ...
  it('draws one beside the creature and one beside the plant', () => {
    const { baseElement } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={note()} place={{ poiId: null } as never} actions={[]} />
    );
  ...
  it('never opens a plate block for a plant, even if one is dropped in', () => {
    ...
    const { baseElement } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}}
        open
        notes={note({
          flora: {
  ...
  it('draws nothing where there is nothing to draw', () => {
    ...
    const { baseElement } = render(
      <Here standing={bare} conversation={null} height="read" onHeight={() => {}} open notes={note({ creature: empty, flora: empty })} place={{ poiId: null } as never} actions={[]} />
    );
